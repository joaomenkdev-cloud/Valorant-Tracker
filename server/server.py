...
def account(name, tag):
    try:
        data = henrik_fetch_safe(
            f"/valorant/v1/account/{quote(name)}/{quote(tag)}",
            TTL["account"]
        )
        acc = data.get("data", {})
        return jsonify({
...
def mmr(region, name, tag):                          # FIX: era 'n', agora 'name'
    try:
        data = henrik_fetch_safe(
            f"/valorant/v2/mmr/{region}/{quote(name)}/{quote(tag)}",  # FIX: era quote(n)
            TTL["mmr"]
        )
...
def mmr_history(region, name, tag):                  # FIX: era 'n', agora 'name'
    try:
        data = henrik_fetch_safe(
            f"/valorant/v1/mmr-history/{region}/{quote(name)}/{quote(tag)}",  # FIX: era quote(n)
            TTL["mmrHistory"]
        )
...
def matches(region, name, tag):                      # FIX: era 'n', agora 'name'
    try:
        mode = request.args.get("mode", "competitive")
        size = min(int(request.args.get("size", 10)), 10)

        data = henrik_fetch_safe(
            f"/valorant/v3/matches/{region}/{quote(name)}/{quote(tag)}?mode={mode}&size={size}",  # FIX
            TTL["matches"]
        )

        player_name = name.lower()   # FIX: era n.lower()
...
def match_detail(match_id):
    try:
        data  = henrik_fetch_safe(f"/valorant/v2/match/{match_id}", TTL["match"])
        m     = data.get("data") or {}
        meta  = m.get("metadata") or {}
        teams = m.get("teams") or {}
...
def assets_agents():
    try:
        ...
        resp = requests.get(
            "https://valorant-api.com/v1/agents?isPlayableCharacter=true", timeout=15
        )
        j = resp.json()
        agents = [
            {
...
if __name__ == "__main__":
    ...
    print(f"   API Key: {'✅ configurada' if API_KEY and API_KEY != 'sua_chave_aqui' else '❌ AUSENTE'}\n")
